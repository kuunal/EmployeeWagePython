import random
from employee_error import employeewage_error
...
class Employee:
    ...
    def calculate_wage_till_total_hour_or_days(self, company_name, wage_per_hour, full_day_hour, monthly_working_day, total_working_hours):
        ...
        while hours > 0 and days > 0:
            ...
            if(wage == self.calculate_fullday_wage(wage_per_hour,full_day_hour)):
                ...
                pass
            days -= 1
        [company for company in self.company_list if company.getCompany_name()==company_name][0].total_and_daily_wage(total_wage,daily_wage)
        return total_wage
    ...
    def get_total_and_daily_wage_of_company(self):
        try:
            if len(self.company_list) == 0:
                raise employeewage_error("No companies registered yet!")
        except employeewage_error as e:
            print("No companies registered yet!")
            self.main()
        company_name = input("Enter company name")
        try:
            company = [company for company in self.company_list if company.getCompany_name()==company_name][0]
        except IndexError:
    ...
    def remove_company(self):
        ...
        try:
            self.company_list.remove([company for company in self.company_list if company.getCompany_name() == company_to_remove][0] )
        except IndexError:
    ...
    def print_companies(self):
        if(len(self.company_list)>0):
            print([company.getCompany_name() for company in self.company_list])
        else:
